...
namespace preprocess
{
    ...
    {
        CPU_NEAREST = 0,         // 最近邻插值CPU
        CPU_BILINEAR = 1,        // 双线性插值CPU
        GPU_NEAREST = 2,         // 最近邻插值GPU
        GPU_NEAREST_CENTER = 3,  // 最近邻插值GPU+中心点对齐
        GPU_BILINEAR = 4,        // 双线性插值GPU
        GPU_BILINEAR_CENTER = 5, // 双线性插值GPU+中心点对齐
        GPU_WARP_AFFINE = 6,     // 仿射变换GPU
    };
    ...
    struct TransInfo
    {
        ...
        int tar_h = 0;
        TransInfo() = default;
        TransInfo(int srcW, int srcH, int tarW, int tarH) : src_w(srcW), src_h(srcH), tar_w(tarW), tar_h(tarH) {}
    };
