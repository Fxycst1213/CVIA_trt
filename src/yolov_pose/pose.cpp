    ...
    namespace pose
    {
            ...
            : Model(onnx_path, level, params)
        {
            ...
            _p3d = (cv::Mat_<double>(10, 3) << -430.6156, -7.47963, -14.7096,
                    ...
                    -317.415, 279.776, 6.3949,
                    -295.229, 164.882, 8.5299,
                    -277.352, 66.6358, 5.4668,
                    -223.665, -4.87759, -25.5137,
                    -284.784, -110.714, 9.0556,
                    -306.016, -224.973, 6.5912,
                    -122.669, -3.90371, -26.3171,
                    0, 0, 0);
        ...
        {
            // --- 核心调参区 ---
            const int search_side = 70;           // ROI 搜索框大小
            const double min_area = 20.0;         // 最小面积过滤 (防止微小噪点)
            const double max_area = 1300.0;       // 最大面积过滤
            const double dist_limit_pixel = 35.0; // 允许偏离 YOLO 初始点的最大像素距离
